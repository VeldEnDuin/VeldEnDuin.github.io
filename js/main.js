(function ($) {
    ...
    */
    ...
    function Viewer(account, albumId) {
        //https://developers.google.com/picasa-web/docs/2.0/developers_guide_protocol
        var ALBUMCACHEKEY = "vd.album",
            ALBUMAPIURI = "http://picasaweb.google.com/data/feed/api",
            THUMBSIZE = 100;
        ...
        function getAlbumCache() {
            if (hasLocalStorage()) {
                return window.localStorage.getItem(ALBUMCACHEKEY) || {};
            }
            return {};
        }
        function putAlbumCache(data) {
            if (hasLocalStorage()) {
                window.localStorage.setItem(ALBUMCACHEKEY, data);
            }
            return data;
        }
        ...
        this.cache = getAlbumCache();

        // load information
    }

    Viewer.prototype.display = function ($album) {
        $album.html('js active... account=' + this.account + ' -- albumId=' + this.albumId);
    ...
    $(function () {
        ...
        if ($album && $album.length === 1) {
            ...
            gpid = data['gp-id'];
            albumid = data['gp-album-id'];

            vwr = new Viewer(gpid, albumid);
            vwr.display($album);
        }
