...
Structuur menu:
MAG EERSTE ITEM NIET VERVANGEN WORDEN DOOR 'HOOFDMENU', 'ITEM' STAAT DAN VOOR DE VERSCHILLENDE SUBMENU'S - NU NOGAL VERWARREND
Het menu bestaat uit hoofdmenu's en submenu's:
Hoofdmenu
MARC: HOE ZET JE ZO'N ACOLADE / MATJE?

```yml WAT WIL DIT ZEGGEN/WAAR DIENT DIT VOOR - IS DIT IETS MET GRIJZE ACHTERGROND TE ZETTEN?'
- item:
    {iso taal code}:
        name: {label}
        link: {link}
